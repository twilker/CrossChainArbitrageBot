...
internal class PancakeSwapTrader : Agent
{
    ...
    {
        if (estimatingGas.GasType != GasType.Trade || estimatingGas.BlockchainName != BlockchainName.Bsc)
        {
            return;
        }

        string sourceChainId = ConfigurationManager.AppSettings["BscId"]
                               ?? throw new ConfigurationErrorsException("BscId not configured");
        string sourceContractAddress = ConfigurationManager.AppSettings["PancakeswapRouterAddress"]
                                      ?? throw new ConfigurationErrorsException("PancakeswapRouterAddress not configured");
        int gas = EstimateGas(sourceChainId, sourceContractAddress);
        OnMessage(new GasEstimatedBasedOnTransactions(estimatingGas, gas, GasType.Trade, BlockchainName.Bsc));
    }
